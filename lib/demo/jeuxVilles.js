...
// Pioche "sans répétition" (comme un jeu de cartes qu'on retourne une
// à une) : mélange `tableau` une fois, distribue un élément à chaque
// appel de .suivant(), et ne remélange qu'une fois le tableau épuisé
// -- ce qui garantit qu'un même élément ne revient jamais avant que
// tous les autres soient passés. Corrige le défaut d'un tirage
// Math.random() à chaque question (qui n'excluait que la toute
// dernière valeur) : sur un pool de 33 villes ou 3 types de question,
// ça faisait revenir les mêmes bien trop souvent et donnait une
// impression de répétition -- voir /jeux/quiz-eclair et
// /jeux/devine-la-ville.
export function creerPioche(tableau) {
  let file = []
  let derniere = null

  function remplir() {
    let nouveau = melanger(tableau)
    // Évite qu'un remélange recommence pile par la valeur qu'on vient
    // de distribuer (répétition "à la jonction" entre deux tours).
    if (nouveau.length > 1 && nouveau[0] === derniere) {
      ;[nouveau[0], nouveau[1]] = [nouveau[1], nouveau[0]]
    }
    file = nouveau
  }
