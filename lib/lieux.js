...
export const TYPES_LIEU = [
  ...
  { valeur: 'attraction', label: 'Attraction', icone: 'camera' },
  { valeur: 'lieu_culte', label: 'Lieu de culte', icone: 'eglise' },
  { valeur: 'loisir', label: 'Loisir', icone: 'ticket' },
...
// Fourchette de prix (1 à 3 -- voir supabase/24_lieux_prix.sql),
// affichée en symboles € comme les guides classiques. `null`/absent :
// personne n'a encore renseigné le prix, on n'affiche rien plutôt
// qu'un "?" ou un "0€" trompeur.
export function prixSymbole(niveau) {
  if (!niveau || niveau < 1) return null
  return '€'.repeat(Math.min(niveau, 3))
}

export const NIVEAUX_PRIX = [
  { valeur: 1, label: '€ -- bon marché' },
  { valeur: 2, label: '€€ -- moyen' },
  { valeur: 3, label: '€€€ -- élevé' },
]
